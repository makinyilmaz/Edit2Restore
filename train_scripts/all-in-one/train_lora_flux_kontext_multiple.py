...
torch.backends.cuda.enable_cudnn_sdp(False)  # cuDNN MHA'yı kapat
...
def log_validation(
    ...
    validation_prompts=None,  # New parameter: list of 4 prompts
):
    # Use the provided validation_prompts or fall back to args.validation_prompt
    if validation_prompts is None:
        if hasattr(args, 'validation_prompts') and args.validation_prompts is not None:
            validation_prompts = args.validation_prompts
        else:
            # Fallback to single prompt for backwards compatibility
            validation_prompts = [args.validation_prompt] if args.validation_prompt else []
    ...
    all_images = []
    all_prompts_for_logging = []
    
    # Iterate through each prompt
    for prompt_idx, prompt in enumerate(validation_prompts):
        logger.info(f"Generating images for prompt {prompt_idx + 1}/{len(validation_prompts)}: '{prompt}'")
        
        # Create a copy of pipeline_args for this prompt
        pipeline_args_cp = pipeline_args.copy()
        
        # Pre-calculate prompt embeds, pooled prompt embeds, text ids
        with torch.no_grad():
            # Remove old prompt if it exists
            pipeline_args_cp.pop("prompt", None)
        ...
        for img_idx in range(args.num_validation_images):
            # Reset generator for reproducibility if seed is set
            if args.seed is not None:
                generator = torch.Generator(device=accelerator.device).manual_seed(args.seed + prompt_idx * 1000 + img_idx)
            ...
            with autocast_ctx:
                ...
                all_prompts_for_logging.append(f"[{prompt_idx}] {prompt}")
...
def parse_args(input_args=None):
    ...
    nargs='+',
    default=None,
    help="List of folder paths containing degradation types (e.g., denoising, deraining, dehazing). Each should have 'clean' and 'degraded' subfolders."
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="List of prompts corresponding to each degradation folder"
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="List of 4 folder paths containing different versions of images"
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="List of 4 prompts corresponding to each version folder"
    )
...
class MultiplePairedDataset(Dataset):
    """
    A dataset to prepare paired images (clean/degraded) with prompts for fine-tuning.
    Supports multiple degradation types (denoising, deraining, dehazing).
    """
    ...
    def __init__(
        ...
        buckets=None,
        args=None,
        # New parameters for degradation training
        degradation_folders=None,  # List of folder paths (e.g., ['denoising', 'deraining', 'dehazing'])
        degradation_prompts=None,  # List of prompts for each degradation type
    ):
        ...
        # Degradation training setup
        if degradation_folders is not None and degradation_prompts is not None:
            assert len(degradation_folders) == len(degradation_prompts), \
                f"Number of folders ({len(degradation_folders)}) must match number of prompts ({len(degradation_prompts)})"
            
            # Load images from all degradation folders
            dataset_entries = []
            
            for folder_path, prompt in zip(degradation_folders, degradation_prompts):
                folder = Path(folder_path)
                if not folder.exists():
                    raise ValueError(f"Folder {folder_path} doesn't exist.")
                
                clean_folder = folder / "clean"
                degraded_folder = folder / "degraded"
                
                if not clean_folder.exists():
                    raise ValueError(f"Clean folder {clean_folder} doesn't exist.")
                if not degraded_folder.exists():
                    raise ValueError(f"Degraded folder {degraded_folder} doesn't exist.")
                
                # Get all images from clean folder
                clean_images = sorted([
                    path for path in clean_folder.iterdir() 
                ...
                # For each clean image, find corresponding degraded image
                for clean_path in clean_images:
                    degraded_path = degraded_folder / clean_path.name
                    
                    if not degraded_path.exists():
                        print(f"  Warning: No matching degraded image for {clean_path.name}, skipping...")
                        continue
                    
                    # Target is clean, condition is degraded
                    # Add the entry 'repeats' times
                    for _ in range(repeats):
                        dataset_entries.append((clean_path, degraded_path, prompt))
                
                print(f"  Added {len(clean_images)} image pairs with prompt: '{prompt}'")
            
            # Print first few entries before shuffling
            print("\n" + "="*100)
            print("BEFORE SHUFFLING - First 8 entries:")
            print("="*100)
            for i, (target, cond, prompt) in enumerate(dataset_entries[:8]):
                print(f"\nEntry {i}:")
                print(f"  Target (clean):     {target}")
                print(f"  Condition (degraded): {cond}")
                print(f"  Prompt:              '{prompt}'")
            print("="*100 + "\n")
            
            # Shuffle the dataset to mix all degradation types
            random.shuffle(dataset_entries)
            
            # Print first few entries after shuffling
            print("\n" + "="*100)
            print("AFTER SHUFFLING - First 8 entries:")
            print("="*100)
            for i, (target, cond, prompt) in enumerate(dataset_entries[:8]):
                print(f"\nEntry {i}:")
                print(f"  Target (clean):     {target}")
            ...
            self.target_paths = []  # Store paths for debugging
            self.cond_paths = []    # Store paths for debugging
            
            for target_path, cond_path, prompt in dataset_entries:
                target_img = Image.open(target_path)
                cond_img = Image.open(cond_path)
                self.instance_images.append(target_img)
                self.cond_images.append(cond_img)
                self.custom_instance_prompts.append(prompt)
                self.target_paths.append(str(target_path))
                self.cond_paths.append(str(cond_path))

        print(f"\nDataset Summary:")
        print(f"  Total instance_images: {len(self.instance_images)}")
        print(f"  Total cond_images: {len(self.cond_images)}")
        print(f"  Total prompts: {len(self.custom_instance_prompts)}")
        ...
        # Counter for batch printing
        self.getitem_counter = 0
        self.print_first_n_batches = 3  # Print first 3 batches
    ...
    def __getitem__(self, index):
        example = {}
        actual_index = index % self.num_instance_images
        
        instance_image, bucket_idx = self.pixel_values[actual_index]
        example["instance_images"] = instance_image
        example["bucket_idx"] = bucket_idx
        
        if self.cond_pixel_values:
            dest_image, _ = self.cond_pixel_values[actual_index]
            example["cond_images"] = dest_image

        if self.custom_instance_prompts:
            caption = self.custom_instance_prompts[actual_index]
            if caption:
        ...
        # Print debug info for first few items in first few batches
        if self.getitem_counter < self.print_first_n_batches * 4:  # 4 items per batch
            print(f"\n{'='*100}")
            print(f"__getitem__ called: counter={self.getitem_counter}, index={index}, actual_index={actual_index}")
            print(f"{'='*100}")
            if hasattr(self, 'target_paths'):
                print(f"  Target (clean):     {self.target_paths[actual_index]}")
                print(f"  Condition (degraded): {self.cond_paths[actual_index]}")
            print(f"  Prompt: '{example['instance_prompt']}'")
            print(f"{'='*100}\n")
    ...
    def paired_transform(self, image, dest_image=None, size=(1024, 1024), random_flip=False):
...
def _iter_val_images(path_or_dir):
...
def main(args):
    ...
    train_dataset = MultiplePairedDataset(
        instance_data_root=args.instance_data_dir,  # Can be None when using degradation_folders
        ...
        args=args,
        degradation_folders=args.degradation_folders,
        degradation_prompts=args.degradation_prompts,
    )
    ...
    has_image_input = (args.cond_image_column is not None) or (
        hasattr(train_dataset, "cond_pixel_values") and len(getattr(train_dataset, "cond_pixel_values") or []) > 0
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                # Collect validation images (single file or a whole directory)
                val_paths = _iter_val_images(args.validation_image) if has_image_input else [None]

                # Where to save
                base_save_dir = os.path.join(args.output_dir, "validation_outputs", f"epoch_{epoch}")
                ...
                for vpath in val_paths:
                    ...
                    cond_img = None
                    tag = "noimg"
                    if has_image_input and vpath is not None:
                        cond_img = load_image(vpath).convert("RGB")
                        pipeline_args["image"] = cond_img
                        tag = Path(vpath).stem  # use file stem for naming
                    ...
                    # save composites: [input | output]
                    save_dir = os.path.join(base_save_dir, tag)
                    os.makedirs(save_dir, exist_ok=True)
                    for i, img in enumerate(images):
                        if cond_img is not None:
                            # resize condition to match output
                            cond_resized = ImageOps.fit(cond_img, img.size, method=Image.BILINEAR)
                            composite = Image.new("RGB", (img.width * 2, img.height))
                            composite.paste(cond_resized, (0, 0))
                            composite.paste(img, (img.width, 0))
                            composite.save(os.path.join(save_dir, f"comp_{i}.png"))
                        # also save raw output
                        #img.save(os.path.join(save_dir, f"gen_{i}.png"))
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            val_paths = _iter_val_images(args.validation_image) if has_image_input else [None]
            base_save_dir = os.path.join(args.output_dir, "validation_outputs", "final")
            os.makedirs(base_save_dir, exist_ok=True)
