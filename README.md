# Edit2Restore

This repository contains the official implementation of the paper:

**Edit2Restore:Few-Shot Image Restoration via Parameter-Efficient Adaptation of Pre-trained Editing Models**  
by M. Akin Yilmaz, Ahmet Bilican, Burak Can Biner and A. Murat Tekalp

[arXiv preprint](https://arxiv.org/abs/2601.03391)

## LoRA Weights
You may download [pre-trained lora adapters](https://drive.google.com/drive/folders/1htnXsBE7zGjkM8AO8bEeNHX0im22bU7a?usp=drive_link) and skip the training process.

## Results
You may directly access and download our [results](https://drive.google.com/drive/folders/1htnXsBE7zGjkM8AO8bEeNHX0im22bU7a?usp=drive_link).

## License
This code is licensed under Apache 2.0.

Note: To use this code, you need FLUX.1 Kontext model weights, 
which are subject to FLUX.1 [dev] Non-Commercial License.
